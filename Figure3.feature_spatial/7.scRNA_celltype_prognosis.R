...
scRNA_markers<-rbind(immunecells_markers,nonimmunecells_markers)
##############
...
names(exp)
names(phe)
gmt.list<-split(scRNA_markers$gene,scRNA_markers$cluster)
#for RFS
##
...
save(scRNA_Scorelist,file = "data/scRNA_Scorelist_allCRC_st_sc.Rdata")
load("data/scRNA_Scorelist_allCRC_st_sc.Rdata")
#for RFS
b<-c()
...
for(set in names(exp)[c(1,3:14,18:19)]){
  ...
  if(nrow(rt)>10){
    for (sig in names(gmt.list)){
      ...
      if (!dir.exists("sc_sp_celltypes_scRNA_rfs_COHORT")){
        dir.create("./sc_sp_celltypes_scRNA_rfs_COHORT")
      }
      if(p.val<0.05){
        pdf(paste0("./sc_sp_celltypes_scRNA_rfs_COHORT/",set,"_",gsub("/","_",sig),"_",HR,"RFS",".pdf"),width = 6.49, height = 6.58,onefile = F )
...
write.csv(colorectal_cox.sig,file="ResultsLM/Figure5_prognosis/cox.sig_scRNA_RFS.csv")###
...
datatable(NMFcoxmeta, 
          ...
          options = list(dom = 'Bfrtip', 
                         buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
write.csv(NMFcoxmeta,file="ResultsLM/Figure5_prognosis/colorectal_cox.sig_scRNA_rfs_meta_results.csv")###
save(NMFcoxmeta,colorectal_cox.sig,file="ResultsLM/Figure5_prognosis/colorectal_cox.sig_scRNA_rfs_meta.Rdata")
##
######forest
library("forestplot")
library("magrittr")
...
pdf(paste0("ResultsLM/Figure5_prognosis/","forestplot_RFS.pdf"),width = 7,height = 5,onefile=FALSE)
...
for(set in names(exp)[c(1,4:9,11:14,18,19)]){
  ...
  if(nrow(rt)>10){
    for (sig in names(gmt.list)){
      ...
      print(paste0(set,sig," p= ",round(p.val,2)," ",HR))
      # pdf(paste("plot", i, ".pdf", sep = ""))
      if (!dir.exists("sc_sp_celltypes_scRNA_os_COHORT")){
        dir.create("./sc_sp_celltypes_scRNA_os_COHORT")
      }
      if(p.val<0.05){
        pdf(paste0("./sc_sp_celltypes_scRNA_os_COHORT/",set,"_",gsub("/","_",sig),"_",HR,"OS",".pdf"),width = 6.49, height = 6.58,onefile = F )
        print(ggsurvplot(kmfit,#surv.median.line = "hv", # Add medians survival
                         # Change legends: title & labels
                         main = "Survival curve",
                         legend.title = paste0(set,"-",sig),
...
#colorectal_cox.sig<-colorectal_cox.sig[colorectal_cox.sig$P.value<0.05,]
######################################################################
write.csv(colorectal_cox.sig,file="ResultsLM/Figure5_prognosis/cox.sig_scRNA_OS.csv")###
#### meta 结果 汇总~ 
library(metafor)
NMFcox<-split(colorectal_cox.sig,colorectal_cox.sig$Characteristics)
...
datatable(NMFcoxmeta, 
          ...
          options = list(dom = 'Bfrtip', 
                         buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
write.csv(NMFcoxmeta,file="ResultsLM/Figure5_prognosis/colorectal_cox.sig_scRNA_OS_meta_results.csv")####
######
######forest
library("forestplot")
library("magrittr")
...
pdf(paste0("ResultsLM/Figure5_prognosis/","forestplot_OS.pdf"),width = 7,height = 5,onefile=FALSE)
forestplot(labeltext=tabletext, #
           mean=c(NA,NA,rt$HR,NA),#HR
           lower=c(NA,NA,rt$low,NA), #95%
           upper=c(NA,NA,rt$up,NA),#95%
