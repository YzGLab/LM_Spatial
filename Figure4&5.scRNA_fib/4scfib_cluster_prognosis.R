...
names(fib.sce.markers.x.sig)<-c("Fib(C1)-ABCC9","Fib(C2)-MYH11", "Fib(C3)-THBS2", 
                                "Fib(C4)-CXCL8", "Fib(C5)-TOP2A")
#sce.cell_type_markers
#sce.markers.epi.sig_st_sc<-sce.markers.sig_st_sc[1:12]
names(exp)
names(phe)
gmt.list<-fib.sce.markers.x.sig
#for RFS
##
scRNA_Scorelist<-list()
...
names(scRNA_Scorelist)
save(scRNA_Scorelist,file = "data/scRNA_Scorelist_allCRC_fib.Rdata")
load("data/scRNA_Scorelist_allCRC_fib.Rdata")
#for RFS
b<-c()
...
for(set in names(exp)[c(1,3:14,18:19)]){
  ...
  if(nrow(rt)>10){
    for (sig in names(gmt.list)){
      ...
      if (!dir.exists("sc_sp_celltypes_scRNA_fib_rfs_COHORT")){
        dir.create("./sc_sp_celltypes_scRNA_fib_rfs_COHORT")
      }
      if(p.val<0.05){
        pdf(paste0("./sc_sp_celltypes_scRNA_fib_rfs_COHORT/",set,"_",gsub("/","_",sig),"_",HR,"RFS",".pdf"),width = 6.49, height = 6.58,onefile = F )
...
write.csv(colorectal_cox.sig,file="ResultsLM/Figure4_fib/cox.sig_scRNA_fib_RFS.csv")###
...
fibcox<-split(colorectal_cox.sig,colorectal_cox.sig$Characteristics)
#names(fibcox)<-c("fib1", "fib2" ,"fib3" , "fib4")
setnames<-names(gmt.list)
fibcoxmeta<-c()
fibcoxlist<-list()
for (set in setnames) {fibcox.i<-fibcox[[set]]
res.rma_each<- metafor::rma(yi = fibcox.i[,4], sei = fibcox.i[,5],method="FE")
results<-round(c(res.rma_each$pval,exp(res.rma_each$b),exp(res.rma_each$ci.lb),exp(res.rma_each$ci.ub)),3)
fibcoxlist[[set]]<-results;
#results<-cbind(set,results)
fibcoxmeta<-rbind(fibcoxmeta,results)}

fibcoxmeta<-data.frame(fibcoxmeta)
colnames(fibcoxmeta)<-c("pvalue","HR","95CI_L","95CI_U")
rownames(fibcoxmeta)<-setnames
head(fibcoxmeta)
library(DT)
datatable(fibcoxmeta,
          options = list(pageLength = 5))
datatable(fibcoxmeta, 
          extensions = 'Buttons',
          options = list(dom = 'Bfrtip', 
                         buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
write.csv(fibcoxmeta,file="ResultsLM/Figure4_fib/colorectal_cox.sig_scRNA_fib_rfs_meta_results.csv")###
save(fibcoxmeta,colorectal_cox.sig,file="ResultsLM/Figure4_fib/colorectal_cox.sig_scRNA_fib_rfs_meta.Rdata")
##
######forest
library("forestplot")
library("magrittr")
...
pdf(paste0("ResultsLM/Figure4_fib/","forestplot_fib_RFS.pdf"),width = 7,height = 5,onefile=FALSE)
...
for(set in names(exp)[c(1,4:9,11:14,18,19)]){
  ...
  if(nrow(rt)>10){
    for (sig in names(gmt.list)){
      ...
      print(paste0(set,sig," p= ",round(p.val,2)," ",HR))
      # pdf(paste("plot", i, ".pdf", sep = ""))
      if (!dir.exists("sc_sp_celltypes_scRNA_fib_os_COHORT")){
        dir.create("./sc_sp_celltypes_scRNA_fib_os_COHORT")
      }
      if(p.val<0.05){
        pdf(paste0("./sc_sp_celltypes_scRNA_fib_os_COHORT/",set,"_",gsub("/","_",sig),"_",HR,"OS",".pdf"),width = 6.49, height = 6.58,onefile = F )
        print(ggsurvplot(kmfit,#surv.median.line = "hv", # Add medians survival
                         # Change legends: title & labels
                         main = "Survival curve",
                         legend.title = paste0(set,"-",sig),
...
#colorectal_cox.sig<-colorectal_cox.sig[colorectal_cox.sig$P.value<0.05,]
######################################################################
write.csv(colorectal_cox.sig,file="ResultsLM/Figure4_fib/cox.sig_scRNA_fib_OS.csv")###
#### meta 结果 汇总~ 
library(metafor)
colorectal_cox.sig<-colorectal_cox.sig[colorectal_cox.sig$numberofpatients>100,]
fibcox<-split(colorectal_cox.sig,colorectal_cox.sig$Characteristics)
#names(fibcox)<-c("C1", "C2" ,"C3","C4")
setnames<-names(gmt.list)
fibcoxmeta<-c()
fibcoxlist<-list()
for (set in setnames) {fibcox.i<-fibcox[[set]]
...
fibcoxmeta<-data.frame(fibcoxmeta)
colnames(fibcoxmeta)<-c("pvalue","HR","95CI_L","95CI_U")
rownames(fibcoxmeta)<-names(gmt.list)

head(fibcoxmeta)
library(DT)
datatable(fibcoxmeta, filter = "top", 
          options = list(pageLength = 5))
datatable(fibcoxmeta, 
          extensions = 'Buttons',
          options = list(dom = 'Bfrtip', 
                         buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
write.csv(fibcoxmeta,file="ResultsLM/Figure4_fib/colorectal_cox.sig_scRNA_fib_OS_meta_results.csv")####
######
######forest
library("forestplot")
library("magrittr")
...
pdf(paste0("ResultsLM/Figure4_fib/","forestplot_fib_OS.pdf"),width = 7,height = 5,onefile=FALSE)
forestplot(labeltext=tabletext, #
           mean=c(NA,NA,rt$HR,NA),#HR
           lower=c(NA,NA,rt$low,NA), #95%
           upper=c(NA,NA,rt$up,NA),#95%
